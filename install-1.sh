...
create_mainfest_file(){
    ...
    cat >  ${SH_PATH}/ps2xu/v2ray-cloudfoundry/manifest.yml  << EOF
    applications:
    - path: .
      name: ${IBM_APP_NAME}
      random-route: true
...
EOF
    ...
    cat >  ${SH_PATH}/ps2xu/v2ray-cloudfoundry/v2ray/config.json  << EOF
    {
        "inbounds": [
            {
                "port": 8080,
...
clone_repo(){
    echo "进行初始化。。。"
    git clone https://github.com/ps2xu/SCCloud
    cd SCCloud
    ...
    chmod 0755 ./*
    cd ${SH_PATH}/SCCloud/v2ray-cloudfoundry
    echo "初始化完成。"
}
...
install(){
    ...
    VMESSCODE=$(base64 -w 0 << EOF
    {
      "v": "2",
      "ps": "SCCloud",
      "add": "SCCloud.us-south.cf.appdomain.cloud",
      "port": "443",
      "id": "${UUID}",
      "aid": "4",
      "net": "ws",
